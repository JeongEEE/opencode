...
export function Session() {
  ...
  createEffect(() => {
    ...
    void (async () => {
      ...
      toast.show({
        message: `${t().session_not_found(route.sessionID)}: ${errorMessage(error)}`,
        variant: "error",
        duration: 5000,
      })
      navigate({ type: "home" })
  ...
  createEffect(() => {
    ...
    const bold = "\x1b[1m"
    const reset = "\x1b[0m"
    const weak = (text: string) => dim + pad(text) + reset
    setEpilogue(
      [
        ``,
        `  ${weak("Session")}${bold}${title}${reset}`,
        `  ${weak("Continue")}${bold}pcode -s ${session()?.id}${reset}`,
        ``,
      ].join("\n"),
    )
  })
...
function Shell(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="$" pending={t().tool_writing_cmd} complete={stringValue(props.input.command)} part={props.part}>
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool
          icon="←"
          pending={t().tool_preparing_write}
...
function Glob(props: ToolProps) {
  ...
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={t().tool_finding_files} complete={stringValue(props.input.pattern)} part={props.part}>
...
function Grep(props: ToolProps) {
  ...
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={t().tool_searching_content} complete={stringValue(props.input.pattern)} part={props.part}>
...
function WebFetch(props: ToolProps) {
  const { t } = useI18n()
  return (
    <InlineTool icon="%" pending={t().tool_fetching_web} complete={stringValue(props.input.url)} part={props.part}>
...
function WebSearch(props: ToolProps) {
  const { t } = useI18n()
  return (
    <InlineTool icon="◈" pending={t().tool_searching_web} complete={stringValue(props.input.query)} part={props.part}>
    ...
    if (!isRunning() && props.part.state.status === "completed") {
      content.push(
        props.metadata.background === true
          ...
          : `└ ${t().task_toolcalls(tools().length)} · ${Locale.duration(duration())}`,
      )
    }
...
function Edit(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="←" pending={t().tool_preparing_edit} complete={stringValue(props.input.filePath)} part={props.part}>
...
function Skill(props: ToolProps) {
  const { t } = useI18n()
  return (
    <InlineTool icon="→" pending={t().tool_loading_skill} complete={stringValue(props.input.name)} part={props.part}>
