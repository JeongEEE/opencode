...
export const ko: Translations = {
  ...
  tips_list: [
    ...
    "OpenCode는 인증이 필요한 원격 MCP 서버의 OAuth를 자동으로 처리합니다",
    ...
    "플러그인을 만들어 OpenCode가 민감한 파일을 읽지 못하도록 방지할 수 있습니다",
    "비대화형 스크립팅에는 {highlight}opencode run{/highlight}을 사용합니다",
    "{highlight}opencode --continue{/highlight}로 마지막 세션을 이어서 실행할 수 있습니다",
    "{highlight}opencode run -f file.ts{/highlight}로 CLI에서 파일을 첨부할 수 있습니다",
    "스크립트에서 기계 읽기 가능한 출력을 얻으려면 {highlight}--format json{/highlight}을 사용합니다",
    "헤드리스 API 접근에는 {highlight}opencode serve{/highlight}를 실행합니다",
    "실행 중인 서버에 연결하려면 {highlight}opencode run --attach{/highlight}를 사용합니다",
    "{highlight}opencode upgrade{/highlight}를 실행하여 최신 버전으로 업데이트할 수 있습니다",
    "{highlight}opencode auth list{/highlight}를 실행하여 설정된 공급자를 모두 확인할 수 있습니다",
    "{highlight}opencode agent create{/highlight}를 실행하면 안내에 따라 에이전트를 만들 수 있습니다",
    "GitHub 이슈/PR에서 {highlight}/opencode{/highlight}를 사용하여 AI 작업을 트리거할 수 있습니다",
    "{highlight}opencode github install{/highlight}을 실행하여 GitHub 워크플로를 설정할 수 있습니다",
    ...
    "{highlight}opencode debug config{/highlight}를 실행하여 설정을 디버깅할 수 있습니다",
