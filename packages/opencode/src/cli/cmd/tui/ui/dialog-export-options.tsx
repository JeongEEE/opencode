...
export function DialogExportOptions(props: DialogExportOptionsProps) {
  ...
  return (
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        <box>
          <text fg={theme.text}>{t().export_filename}</text>
        </box>
        <textarea
          onSubmit={() => {
            props.onConfirm?.({
          ...
          placeholder={t().export_filename_placeholder}
      ...
      <box flexDirection="column">
        ...
        >
          ...
          <text fg={store.active === "thinking" ? theme.primary : theme.text}>{t().export_include_thinking}</text>
        </box>
        <box
          flexDirection="row"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "toolDetails" ? theme.primary : theme.text}>{t().export_include_details}</text>
        </box>
        <box
          flexDirection="row"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "assistantMetadata" ? theme.primary : theme.text}>{t().export_include_metadata}</text>
        </box>
        <box
          flexDirection="row"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "openWithoutSaving" ? theme.primary : theme.text}>{t().export_open_nosave}</text>
        </box>
      </box>
      <Show when={store.active !== "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t().export_hint_toggle("space", "return")}
        </text>
      </Show>
      <Show when={store.active === "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t().export_hint_confirm("return", "tab")}
        </text>
      </Show>
    </box>
  )
