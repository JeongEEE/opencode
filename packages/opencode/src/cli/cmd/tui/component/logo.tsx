...
const CELLS = DEFAULT.FULL.flatMap((line, y) => Array.from(line).flatMap((char, x) => (lit(char) ? [{ x, y }] : [])))
      ...
      <For each={ctx.shape.left}>
        {(line, index) => (
          <box flexDirection="row" gap={1}>
            <box flexDirection="row">{renderLine(line, index(), props.ink ?? pastelPurple, false, 0, frame(), dusk(), idleState())}</box>
