...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const upgradeCurl = Effect.fnUntraced(function* (target: string) {
      const response = yield* httpOk.execute(HttpClientRequest.get("https://github.com/JeongEEE/opencode/releases/latest/download/install"))
      const body = yield* response.text
      const bodyBytes = new TextEncoder().encode(body)
      const upgradeResult = yield* appProcess.run(
      ...
      return {
        ...
        stdout: upgradeResult.stdout.toString("utf8"),
        stderr: upgradeResult.stderr.toString("utf8"),
      }
    }, Effect.orDie)
